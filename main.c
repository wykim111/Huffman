...
int main()
{
	...
	in = fopen("C:\\Users\\ksq11\\source\\repos\\huffman\\huffman\\input.txt", "r");

	if (in == NULL)
	{
		printf("input.txt not opened\n");

		return -1;
	}

	out = fopen("C:\\Users\\ksq11\\source\\repos\\huffman\\huffman\\output.txt", "w");

	if (out == NULL)
	{
		printf("output.txt not opened\h");
